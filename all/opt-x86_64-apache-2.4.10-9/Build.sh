...
PKG=opt-x86_64-apache-$APACHEVER-9 # with build version
...
pkg_install musl-openssl-1.0.1n-1 || exit 2
pkg_install musl-apr-1.5.1-1 || exit 2
pkg_install musl-apr-util-1.5.3-1 || exit 2
pkg_install musl-pcre-8.30-2 || exit 2
pkg_install musl-libxml2-2.9.0-2 || exit 2
pkg_install musl-1.1.10-1 || exit 2 
export CC=musl-gcc
...
patch -p0 < $PKGDIR/bugfix-2.4.10.pat || exit 1
